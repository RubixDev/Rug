...
public abstract class PlayerEntityMixin extends LivingEntity {
    ...
    //$$ private Difficulty onTickMovement(Difficulty original) {
    ...
    //$$ private boolean onTickMovement(boolean original) {
    ...
    private boolean infinityNeedsArrow(boolean original, ItemStack bow) {
        return original || (
                !RugSettings.infinityNeedsArrow
                        //#if MC >= 12101
                        && EnchantmentHelper.getLevel(getRegistryManager().getOrThrow(RegistryKeys.ENCHANTMENT).getOrThrow(Enchantments.INFINITY), bow) > 0
                        //#else
